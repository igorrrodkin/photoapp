...
export const createAlbumDB = async (
  ...
  const client = await db.connect();
  await client.insert(albums).values({
    login: login,
    albumName: albumName,
    albumLocation: location,
    datapicker: datapicker,
...
export const listAlbums = async (login: string) => {
  const client = await db.connect();
...
export const giveAccessToAlbum = async (
  ...
  const client = await db.connect();
  const accessIsAlreadyGiven = await client
    .select(albums)
    .fields({ access_clients: albums.accessClients })
    .where(
      and(eq(albums.login, loginPhotographer), eq(albums.albumName, albumName))
    );
  const accessContent = accessIsAlreadyGiven[0].access_clients;
  let finalArray: string[];
  if (accessContent) {
  ...
  await client
    ...
    .set({ accessClients: JSON.stringify(finalArray) })
    .where(
      and(eq(albums.albumName, albumName), eq(albums.login, loginPhotographer))
    );
};

export const availableAlbums = async (userLogin: string): Promise<Album[]> => {
  const client = await db.connect();
  const content = await client.select(albums);
  const availableAlbums = content.filter(
    (item) =>
      item.accessClients && JSON.parse(item.accessClients).includes(userLogin)
  );
...
const al = new Albums(client);

al.ifAlbumExists("photographer_", "albumthebest").then((res) =>
  console.log(res)
);
